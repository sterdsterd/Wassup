...
class DetailAdapter(val activity: DetailActivity, val items : MutableList<MemberData>, val taskName: String) : RecyclerView.Adapter<DetailAdapter.MainViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MainViewHolder, position: Int) {
        ...
        holder.stat.text = "RSSI : ${items[position].rssi} / 약 ${Math.pow(10.0, (-59.0 - items[position].rssi) / (10 * 2))}m"
        ...
        if (items[position].isDetected) {
            holder.cardTint.background = ContextCompat.getDrawable(activity, R.drawable.gradient_card)
            items[position].isNotified = false
        }
        else {
            ...
            if (!items[position].isNotified) {
                items[position].isNotified = true
                ...
                var notificationBuilder = NotificationCompat.Builder(activity, CHANNEL_ID)
                    ...
                    .setContentText("${items[position].name}이 벗어났습니다")
                    .setAutoCancel(true)
